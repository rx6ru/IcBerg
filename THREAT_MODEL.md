...
| **Thread-safe `ApprovalQueue`/`AuditLog` connections + audit-coupled execution** (`approvals.py`/`audit.py`, a per-instance `threading.RLock` guarding every `self.conn` access; `ApprovalQueue.approve` holds it across claim+execute+audit-append as ONE critical section) | R13 (hardening) | H1 fix. Sync FastAPI routes run in Starlette's threadpool, so the "SQLite serializes writes on one connection" claim above was incomplete: it protects against a corrupted on-disk write, not against two Python threads calling methods on the SAME `sqlite3.Connection`/cursor object concurrently, which previously threw `InterfaceError`/`OperationalError`/bare-`AssertionError` as unhandled 500s and, worse, could let two callers both read the same `AuditLog` `prev_hash`/`seq` before either committed (hash-chain corruption), or let a write execute via `write_executor` in `approve()` with the process interrupted before the `audit_log.append(...)` immediately after it ever ran (an executed-but-unaudited write — the literal auditability-guarantee break this fix closes). The lock makes every `self.conn` access — including `GET /health`'s connectivity probe, now routed through a `ping()` method on each class rather than touching `.conn` directly — mutually exclusive, and `AuditLog.append` runs its `_next_seq`/`_last_entry_hash` read through its `INSERT`+commit+anchor-write as one lock acquisition so a concurrent append can never observe a torn predecessor. Regression-tested by `tests/api/test_concurrency.py`: N=50 concurrent `approve` calls on one held write yield exactly 1 execution, 0 unhandled 500s, `AuditLog.verify() == True`, and an `approved`-entry count that exactly equals the execution count; N=50 concurrent DISTINCT approvals all succeed, are all individually audited, and leave a valid chain |
...
| **Self-approval blocked** (`ApprovalQueue.approve`, `approver == proposer` check before `_claim`) | R12, R14 (partial) | `approve()` refuses (403, `ApprovalError`) before ever claiming the row if `approver` equals the `actor` who originally proposed the SQL — the same identifier cannot propose a write and then authorize its own execution. Compared via `_same_identity` (strip + casefold, M2 hardening) rather than bare `==`, so `approver="agent-1 "` (a trailing space) or `"AGENT-1"` (a case variant) cannot bypass the check the exact string `"agent-1"` would trip — regression-tested by the trailing-space/case-variant parametrization in `TestSelfApproval`. This is an in-process stopgap given the identity gap below (R14 remains open for a DIFFERENT claimed identity), not a full fix for approver/actor spoofing — see "Phase 2 Residual Risks". Regression-tested by `TestSelfApproval` (P2.16) |
| **PII values redacted from `/audit`'s `matched_rules` and from `GET /approvals`'s `sql`/`reason`/`actor`** (`gateway_routes.get_audit`/`list_approvals`, `redact_text` applied to every free-text field) | R3 (extended to two more API surfaces) | M3/M4 hardening. `approve`/`reject` fold `f"proposed_by={record.actor}"` into `matched_rules`, and `actor` there is the caller-supplied, unredacted `QueryRequest.actor` — an email-shaped actor previously reached `/audit` raw through this field even though the entry's own top-level `actor` was already redacted next to it. Separately, `GET /approvals` echoed a pending write's `sql`/`reason`/`actor` with NO redaction at all, so a PII literal embedded in a held statement (`WHERE email='alice@example.com'`) was fully visible to anyone who could reach the endpoint. Both now run every free-text field through the same `redact_text` scrub `proposed_sql`/`actor` already get, preserving query STRUCTURE (table/column/clause shape) for an approver's judgment while masking the literal value. Regression-tested by the M3/M4 additions to `TestAuditEndpoint`/`TestApprovalsListRedaction` (`tests/api/test_gateway_api.py`) |
...
- **`GET /approvals`'s PII-value redaction narrows what an unauthorized viewer sees; it
  does not substitute for auth-gating the endpoint.** M4 (`gateway_routes.list_approvals`)
  runs `redact_text` over `sql`/`reason`/`actor` before they leave this endpoint, so a
  PII literal embedded in a pending write (`WHERE email='alice@example.com'`) is no
  longer visible to anyone who can reach it. This is a confidentiality mitigation for the
  UNAUTHENTICATED-reader case, not a replacement for real access control — the same
  identity-provider gap this section documents everywhere else applies here too: in
  production, `GET /approvals` (like `POST /approvals/{id}`) MUST sit behind real
  authentication before it is exposed beyond a trusted internal network, and once an
  approver IS authenticated and authorized, they legitimately need the FULL, un-redacted
  SQL to make an informed approve/reject decision — a real deployment should route an
  authorized approver's `GET /approvals` view around this redaction (or supply it as a
  separate, privileged field) rather than have this convenience-layer scrubbing degrade
  the approval decision itself. Redacting by default here is the safer failure mode for
  today's unauthenticated endpoint, not the intended long-term shape of the feature.
